...
import com.minko.socket.dto.LoginRequest;
import com.minko.socket.dto.LoginResponse;
import com.minko.socket.dto.RefreshTokenRequest;
...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponse google(SocialRequest socialRequest) {
        try {
            GoogleIdToken googleIdToken = GoogleIdToken
                .parse(builder.getJsonFactory(), socialRequest.getToken());
            ...
            LoginRequest loginRequest = new LoginRequest(account.getEmail(),
                secretPassword);
            log.info("In google - loginRequest: {} successfully created",
                loginRequest);
            return loginAccount(loginRequest);
        } catch (IOException e) {
            log.error(
                "In google - Exception while parsing google token. Message: {} ",
                e.getMessage());
        }
        return null;
    }
    ...
    public LoginResponse facebook(SocialRequest socialRequest) {
        ...
        Account loadedAccount = facebook
            .fetchObject("me", Account.class, "email");
        String email = loadedAccount.getEmail();
        Account account = findOrCreateSocialAccount(email, socialRequest);
        LoginRequest loginRequest = new LoginRequest(account.getEmail(),
            secretPassword);
        log.info("In facebook - loginRequest: {} successfully created",
            loginRequest);
        return loginAccount(loginRequest);
    }

    @Transactional(readOnly = true)
    public Account findOrCreateSocialAccount(String email,
        SocialRequest socialRequest) {
        Account account = null;
        if (accountService.existsByEmail(email)) {
            account = accountService.getByEmail(email);
            log.info(
                "In findOrCreateSocialAccount - account: {} successfully logined",
                account);
        } else {
            ...
            log.info("In findOrCreateSocialAccount - registrationRequest:"
                + " {} successfully created", registrationRequest);
        }
        return account;
    }

    @Transactional
    public Account createSocialAccount(
        RegistrationRequest registrationRequest) {
        Role roleUser = roleRepository.findByRoleType(RoleType.ROLE_USER)
            .orElseThrow(() -> new SocketException(
                "Role not found with role type - " + RoleType.ROLE_USER));
        List<Role> roles = Collections.singletonList(roleUser);
        String bcryptedPassword = passwordEncoder
            .encode(registrationRequest.getPassword());
        Account account = accountMapper
            .mapDtoToAccount(registrationRequest, roles, bcryptedPassword);
        account.setEnabled(true);
        Account savedAccount = accountRepository.save(account);
        log.info("IN createSocialAccount - account: {} successfully created",
            savedAccount);
        return savedAccount;
    }

    @Transactional
    public RegistrationResponse createCustomAccount(
        RegistrationRequest registrationRequest) {
        Role roleUser = roleRepository.findByRoleType(RoleType.ROLE_USER)
            .orElseThrow(() -> new SocketException(
                "Role not found with role type - " + RoleType.ROLE_USER));
        ...
        mailService.sendMail(
            new NotificationEmail("Please activate your account",
                ...
                + "http://localhost:4200/verification/" + verificationToken
                .getToken()));
        RegistrationResponse registrationResponse = accountMapper
            .mapFromAccountToDto(account);
        log.info(
            "In createCustomAccount - registrationResponse: {} successfully created",
    ...
    public void verifyAccount(String token) {
        VerificationToken verificationToken = verificationTokenRepository
            .findByToken(token).orElseThrow(() -> new SocketException(
                "Verification token doesn't exit - " + token));
        ...
        log.info(
            "In verifyAccount - account: {} is enabled, verificationToken:"
    ...
    public VerificationToken generateVerificationToken(Account account) {
        ...
        verificationToken
            .setExpirationDate(Instant.now().plusMillis(verifyTokenExp));
        VerificationToken savedVerificationToken = verificationTokenRepository
            .save(verificationToken);
        log.info(
            "In generateVerificationToken - verificationToken: {} successfully created",
            verificationToken);
    ...
    public LoginResponse loginAccount(LoginRequest loginRequest) {
        ...
        Instant expiresAt = Instant.now()
            .plusMillis(jwtProvider.getExpiration());
        LoginResponse loginResponse = accountMapper
            .mapToLoginResponse(getCurrentAccount(), authToken, refreshToken,
                expiresAt);
        log.info("In loginAccount - loginResponse: {} successfully created",
            loginResponse);
    ...
    public Account getCurrentAccount() {
        ...
        log.info("In getCurrentAccount account: {} successfully returned",
            account);
        return account;
    }
    ...
    public String login(LoginRequest loginRequest) {
        ...
        log.info("In login - token: {} successfully created at: {}", token,
            Instant.now());
        return token;
    }
    ...
    public LoginResponse refreshToken(RefreshTokenRequest refreshTokenRequest) {
        Account account = accountService
            .getByEmail(refreshTokenRequest.getEmail());
        refreshTokenService
            .validateRefreshToken(refreshTokenRequest.getRefreshToken());
        String authToken = jwtProvider
            .generateTokenByEmail(refreshTokenRequest.getEmail());
        Instant expiresAt = Instant.now()
            .plusMillis(jwtProvider.getExpiration());
        LoginResponse loginResponse = accountMapper
            .mapToLoginResponse(account, authToken,
                refreshTokenRequest.getRefreshToken(), expiresAt);
        log.info("In refreshToken - loginResponse: {} successfully created",
            loginResponse);
        return loginResponse;
    }
}
